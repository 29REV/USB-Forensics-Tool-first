...
class DevicesPage(ttk.Frame):
    def __init__(self, parent, app):
        ...
        self._scan_error = ""
        self._create_page()
        # Defer initial scan until Tk main loop is running.
        self.after(200, self._scan_all_devices)
    ...
    def _scan_all_devices(self):
        ...
        def scan_thread():
            ...
            try:
                ...
                # Background thread stores results only; main thread applies results.
                self._scan_results = deduped
                self._scan_error = error_message
                self._scan_done = True

        thread = threading.Thread(target=scan_thread, daemon=True)
        thread.start()
        self._schedule_scan_apply()

    def _schedule_scan_apply(self):
        if not self.scanning:
            return
        if self._scan_done:
            self._apply_scan_results()
            return
        self._scan_job = self.after(100, self._schedule_scan_apply)

    def _apply_scan_results(self):
        try:
            self.device_tree.delete(*self.device_tree.get_children())
            self.current_devices = self._scan_results

            seen_rows = set()
            for i, device in enumerate(self._scan_results, 1):
            ...
            if self._scan_error:
                messagebox.showerror("Error", f"Failed to scan devices: {self._scan_error}")
        finally:
            self.scanning = False
            self._scan_done = False
            try:
