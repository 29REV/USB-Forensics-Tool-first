...
def summarize(devices: List[object]) -> List[dict]:
    ...
    for d in devices:
        rec = {
            ...
            'wireshark_analysis': getattr(d, 'wireshark_analysis', {}) or {},
            'communication_summary': getattr(d, 'communication_summary', ''),
        }
        out.append(rec)
...
def detect_suspicious(summaries: List[dict]) -> List[Tuple[dict, str]]:
    ...
    for s in summaries:
        ...
        wireshark_analysis = s.get('wireshark_analysis', {}) or {}
        if isinstance(wireshark_analysis, dict):
            for pattern in wireshark_analysis.get('suspicious_patterns', []) or []:
                if pattern not in reasons:
...
def compute_anomaly_score(device: Dict[str, Any]) -> float:
    ...
    connections = int(device.get('connections', device.get('total_connections', 0)) or 0)
    if connections > 10:
        score += min(15, connections)

    wireshark_analysis = device.get('wireshark_analysis', {}) or {}
    if wireshark_analysis and isinstance(wireshark_analysis, dict):
        score += min(30, float(wireshark_analysis.get('suspicious_score', 0) or 0) * 0.30)
        if wireshark_analysis.get('suspicious_patterns'):
            score += 10
    
    return min(100.0, score)


def merge_trace_analysis(device: Dict[str, Any], wireshark_analysis: Dict[str, Any]) -> Dict[str, Any]:
    """Attach optional Wireshark-derived findings to a device summary."""
    summary = dict(device) if isinstance(device, dict) else asdict(device)
    summary['wireshark_analysis'] = wireshark_analysis or {}
    summary['communication_summary'] = (wireshark_analysis or {}).get('next_level_summary', '')
    summary['anomaly_score'] = compute_anomaly_score(summary)
    return summary
...
def enrich_summary(device: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary["wireshark_analysis"] = summary.get("wireshark_analysis", {}) or {}
    summary["communication_summary"] = summary.get("communication_summary") or summary["wireshark_analysis"].get("next_level_summary", "")
    summary["anomaly_score"] = compute_anomaly_score(summary)
