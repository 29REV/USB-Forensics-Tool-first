...
def get_all_usb_devices_wmi() -> List[USBDevice]:
    ...
    try:
        ...
        # Build driver lookup map once, then enrich PnP device rows.
        try:
            driver_query = (
                "SELECT DeviceID, DriverVersion, DriverProviderName, DriverDate "
                "FROM Win32_PnPSignedDriver"
            )
            for drv in service.ExecQuery(driver_query):
                dev_id = str(getattr(drv, "DeviceID", "") or "").strip()
                if not dev_id:
                    continue
                driver_map[dev_id.lower()] = {
                    "driver_version": str(getattr(drv, "DriverVersion", "") or ""),
                    "driver_provider": str(getattr(drv, "DriverProviderName", "") or ""),
                    "driver_date": str(getattr(drv, "DriverDate", "") or ""),
                }
        except Exception as e:
            logger.debug(f"Could not query Win32_PnPSignedDriver: {e}")
        ...
        for device in usb_devices:
            try:
                ...
                driver_version = ""
                driver_info = driver_map.get(device_id.lower())
                if driver_info:
