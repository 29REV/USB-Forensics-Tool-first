...
def _earliest(existing: Any, candidate: Any) -> Any:
    if existing is None:
        return candidate
    if candidate is None:
        return existing
    return min((existing, candidate), key=_timestamp_key)


def _latest(existing: Any, candidate: Any) -> Any:
    if existing is None:
        return candidate
    if candidate is None:
        return existing
    return max((existing, candidate), key=_timestamp_key)
...
def correlate(registry_entries: List[object], event_entries: List[object]) -> List[DeviceRecord]:
    ...
    for r in registry_entries:
        ...
        rec = DeviceRecord(
            ...
            first_seen=getattr(r, 'last_write', None),
    ...
    for e in event_entries:
        ...
        for rec in records.values():
            if rec.serial and rec.serial in (name or ''):
                rec.connections += 1
                rec.last_seen = _latest(rec.last_seen, ts)
                rec.first_seen = _earliest(rec.first_seen, ts)
